...
tmux split-window -v -l 2  "export message=\"PING $server_name ($server_host)\" && source \"$ATOMIQUE_ROOT_DIR/inc/inc_message_and_wait.sh\" && tmux send-keys -t 0 C-c C-c && tmux send-keys -t 1 C-c && exit" 


#tmux send-keys -t 1 "gping $server_host && exit 0" C-m
#tmux send-keys -t 2 "export message=\"PING $server_name ($server_host)\" && source \"$ATOMIQUE_ROOT_DIR/inc/inc_message_and_wait.sh\" && tmux send-keys -t 0 C-c C-c && tmux send-keys -t 1 C-c && exit" C-m
